...
func (ks *KafkaService) processMessage(ctx context.Context, message kafka.Message, processor NotificationProcessor) error {
	...
	log.Printf("Processing notification: %s for recipient: %s", payload.NotificationID, payload.To)
	...
	return ks.SendAck(payload.NotificationID, models.StatusSuccess, "WhatsApp message sent successfully")
}
